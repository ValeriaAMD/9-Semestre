...
origen = "C:/Users/caroc/Untitled Folder/geopandas/shapes"
salida = "C:/Users/caroc/Untitled Folder/geopandas/shapes/COMPLETO"
...
for nombrezip in os.listdir(origen):
    for i in range(2010, 2022):
        for j in range(1, 13):
            ...
            for result in [result0, result1, result2, result3, result4]:
                # Validar si el GeoDataFrame resultante es vacío o tiene geometrías inválidas
                if result.empty or not result['geometry'].is_valid.all():
                    print(f"Skipping invalid intersection result for {nombrezip}")
                    continue
            ...
            geometry = [Point(xy) for xy in zip(seq_finales['lons'], seq_finales['lats'])]
            gdf_seq_finales = gpd.GeoDataFrame(seq_finales, geometry=geometry)

            # Guardar el GeoDataFrame en un archivo shapefile
            gdf_seq_finales.to_file(os.path.join(salida, f"{nombrezip}.shp"), encoding="utf-8")

            # Limpiar el GeoDataFrame para la próxima iteración
            sequia = gpd.GeoDataFrame()
